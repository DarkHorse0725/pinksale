...
contract Presale is ReentrancyGuard {
    ...
    external payable nonReentrant {
        ...
        uint256 tokensSold = amount_in * presaleInfo.presaleRate / (10 ** uint256(presaleInfo.baseToken.decimals()));
    ...
    function userWithdrawTokens () external nonReentrant {
        ...
        uint256 tokensOwed = presaleInfo.presaleToken.balanceOf(address(this)) * (buyer.tokensOwed / tokensRemainingDenominator);
        require(tokensOwed > 0, 'NOTHING TO WITHDRAW');
        status.TOTAL_TOKENS_WITHDRAWN = status.TOTAL_TOKENS_WITHDRAWN + buyer.tokensOwed;
        buyer.tokensOwed = 0;
    ...
    function userWithdrawBaseTokens () external nonReentrant {
        ...
        require(tokensOwed > 0, 'NOTHING TO WITHDRAW');
        status.TOTAL_BASE_WITHDRAWN = status.TOTAL_BASE_WITHDRAWN + buyer.baseDeposited;
    ...
    function ownerWithdrawTokens () external onlyPresaleOwner {
        ...
        presaleInfo.presaleToken.safeTransfer(presaleInfo.presaleOwner, presaleInfo.presaleToken.balanceOf(address(this)));
    }
    ...
    function ownerForceSuccess () external onlyPresaleOwner {
        ...
        require(presaleInfo.hardcap < status.TOTAL_BASE_COLLECTED + presaleInfo.minSpendPerBuyer);
    ...
    function isPresaleTaxless() internal returns (bool) {
        ...
        presaleInfo.presaleToken.safeTransfer(address(_taxCheck), _tokenBalanceBefore);
    ...
    function addLiquidity() external nonReentrant {
        ...
        uint256 baseLiquidity = status.TOTAL_BASE_COLLECTED * presaleInfo.liquidityPercent / 1000;
        
            WETH.deposit{value : baseLiquidity}();
        
        presaleInfo.baseToken.approve(address(smartLockForwarder), baseLiquidity);
        
        // sale token liquidity
        uint256 tokenLiquidity = baseLiquidity * presaleInfo.listingRate / (10 ** uint256(presaleInfo.baseToken.decimals()));
        presaleInfo.presaleToken.approve(address(smartLockForwarder), tokenLiquidity);
        ...
        if (remainingSBalance > status.TOTAL_TOKENS_SOLD) {
            ...
            presaleInfo.presaleToken.safeTransfer(0x000000000000000000000000000000000000dEaD, burnAmount);
        }
